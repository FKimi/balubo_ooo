...
class AnalysisTimeTracker {
  ...
  recordAnalysisEnd(_sessionId: string) {
    try {
      ...
      const { contentType, contentLength, hasImages, startTime } =
  ...
  predictAnalysisTime(
    ...
    // 類似の記録をフィルタリング
    const similarRecords = this.records.filter(
      (record) =>
    ...
    const weightedSum = similarRecords.reduce(
      (sum, record, index) =>
  ...
  getStatistics() {
    ...
    const averageTime =
      this.records.reduce((sum, record) => sum + record.analysisTime, 0) /
      totalRecords /
      1000;

    const contentTypeStats = this.records.reduce(
      (stats, record) => {
...
// 便利な関数
export function startAnalysisTracking(
  contentType: string,
  contentLength: number,
  hasImages: boolean,
): string {
  return analysisTimeTracker.recordAnalysisStart(
    contentType,
    contentLength,
    hasImages,
  );
...
export function predictAnalysisTime(
  contentType: string,
  contentLength: number,
  hasImages: boolean,
): number {
  return analysisTimeTracker.predictAnalysisTime(
    contentType,
    contentLength,
    hasImages,
  );
