...
export function getAuthToken(req: NextRequest): string | null {
  ...
  console.log(
    "[API-Utils] Authorization Header:",
    authHeader ? "存在します" : "存在しません",
    authHeader,
  );
  if (!authHeader) {
    return null;
  ...
  console.log(
    "[API-Utils] Extracted Token (first 10 chars):",
...
export async function withAuth<T>(
  ...
  try {
    ...
    console.log(
      "[API-Utils] withAuth: Token from getAuthToken:",
      token ? token.substring(0, 10) + "..." : "なし",
    );
    if (!token) {
    ...
    console.log("[API-Utils] withAuth: Supabase getUser結果:", {
      userExists: !!user,
      authError: error?.message,
    });
    if (error || !user) {
      console.error(
        "[API-Utils] withAuth: 認証検証エラー:",
...
export async function withOptionalAuth<T>(
  ...
  try {
    ...
    console.log(
      "[API-Utils] withOptionalAuth: Token from getAuthToken:",
      token ? token.substring(0, 10) + "..." : "なし",
    );
    ...
    if (token) {
      try {
        ...
        console.log("[API-Utils] withOptionalAuth: Supabase getUser結果:", {
          userExists: !!user,
          authError: error?.message,
        });
        if (!error && user) {
        ...
        // 認証エラーでも処理を続行（匿名アクセス）
        console.warn(
          "[API-Utils] withOptionalAuth: 認証トークンが無効ですが、匿名アクセスとして続行します:",
          authError,
        );
      }
    }
 ...
 */
export async function extractId(context: {
