...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      // 作成に成功した場合は削除
      if (insertResult) {
        await authClient.from("profiles").delete().eq("id", insertResult.id);
    ...
    try {
      const { data: workInsertResult, error: workInsertError } =
        await authClient.from("works").insert(testWorkData).select().single();
      ...
      // 作成に成功した場合は削除
      if (workInsertResult) {
        await authClient.from("works").delete().eq("id", workInsertResult.id);
    ...
    const rlsAnalysis = {
      profiles: {
        ...
        insertPolicy: results.tests.profileInsert.error
          ? `INSERT権限に問題: ${results.tests.profileInsert.error}`
          : "INSERT権限は正常",
        recommendation:
          results.tests.profileInsert.errorCode === "42501"
      ...
      works: {
        ...
        insertPolicy: results.tests.workInsert.error
          ? `INSERT権限に問題: ${results.tests.workInsert.error}`
          : "INSERT権限は正常",
        recommendation:
          results.tests.workInsert.errorCode === "42501"
            ? "RLSポリシーでINSERT操作が拒否されています。auth.uid() = user_id のポリシーが必要です。"
            : "権限は正常に設定されています",
      },
    };
