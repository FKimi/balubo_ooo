  ...
  try {
    ...
    if (authHeader && userInfo && !userInfo.error) {
      ...
      try {
        ...
        // profilesテーブル（認証）
        const { data: authProfilesData, error: authProfilesError } =
          await authClient
            .from("profiles")
            .select("*")
    ...
    const rlsAnalysis = {
      profiles: {
        hasRLS:
          tablesInfo.profiles.error?.includes("row-level security") ||
      ...
      works: {
        hasRLS:
          tablesInfo.works.error?.includes("row-level security") ||
