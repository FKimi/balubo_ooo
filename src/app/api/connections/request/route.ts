...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!connectionId || !action) {
      return NextResponse.json(
        { error: "connectionIdとactionが必要です" },
        { status: 400 },
      );
    }
    ...
    if (!["approve", "decline"].includes(action)) {
      return NextResponse.json(
        { error: "actionはapproveまたはdeclineである必要があります" },
        { status: 400 },
      );
    }
