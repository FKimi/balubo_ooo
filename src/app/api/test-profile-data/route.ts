...
export async function POST(request: NextRequest) {
  try {
    ...
    const testProfileData = {
      ...
      introduction:
        "これは詳細な自己紹介のテストです。この機能が正しく動作しているかを確認しています。",
    ...
    // プロフィールを保存
    const savedProfile = await DatabaseClient.saveProfile(
      user.id,
      testProfileData,
      token,
    );
    ...
    const testResults = {
      ...
      fieldComparison: {
        displayName: {
          ...
          matched:
            testProfileData.displayName === retrievedProfile?.display_name,
        },
        bio: {
        ...
        introduction: {
          ...
          matched:
            testProfileData.introduction === retrievedProfile?.introduction,
        },
        avatarImageUrl: {
          sent: testProfileData.avatarImageUrl,
          saved: retrievedProfile?.avatar_image_url,
          matched:
            testProfileData.avatarImageUrl ===
            retrievedProfile?.avatar_image_url,
        },
        backgroundImageUrl: {
          sent: testProfileData.backgroundImageUrl,
          saved: retrievedProfile?.background_image_url,
          matched:
            testProfileData.backgroundImageUrl ===
            retrievedProfile?.background_image_url,
        },
      },
    };
