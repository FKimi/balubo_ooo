...
export async function GET(request: NextRequest, context: any) {
  ...
  try {
    ...
    console.log(
      `[${requestId}] GET /api/works/[id] - ID: ${id}, Auth: ${!!authHeader}`,
    );
    ...
    if (!work) {
      try {
        ...
        const supabaseForPublic = createClient(
          supabaseUrl,
          supabaseServiceKey,
          {
            auth: {
        ...
        const { data: supabaseWork, error: supabaseError } =
          await supabaseForPublic
            .from("works")
            .select("*")
            .eq("id", id)
            .single();
        ...
        if (supabaseError) {
          console.log(
            `[${requestId}] Supabase検索エラー:`,
            supabaseError.message,
          );
          console.log(`[${requestId}] Supabase検索エラー詳細:`, supabaseError);
        } else if (supabaseWork) {
          console.log(
            `[${requestId}] Supabaseで作品が見つかりました:`,
            supabaseWork.title,
          );
          console.log(`[${requestId}] 作品データ:`, {
            id: supabaseWork.id,
    ...
    if (!work) {
      console.log(
        `[${requestId}] どこからも作品が見つかりませんでした ID:`,
        id,
      );
      ...
      try {
        ...
        const { data: supabaseWorks, error: _supabaseWorksError } =
          await supabaseForIds.from("works").select("id").limit(10);
    ...
    console.log(
      `[${requestId}] 成功: 作品データを返却 - 処理時間: ${endTime - startTime}ms`,
    );
...
export async function PUT(request: NextRequest, context: any) {
  try {
    ...
    if (!roles || roles.length === 0) {
      return NextResponse.json({ error: "役割は必須です" }, { status: 400 });
    }
    ...
    if (error || !work) {
      ...
      if (
        error?.code === "42703" ||
        error?.message?.includes("production_notes")
      ) {
        console.log(
          "production_notesカラムが存在しないため、除外して再試行します",
        );
