      ...
      {topTags.length > 0 && (
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-[0_18px_45px_rgba(15,23,42,0.06)]">
          <div className="flex items-start gap-3 mb-5">
            {/* クレイモーフィズム風アイコンコンテナ */}
            <div className="w-12 h-12 rounded-xl bg-emerald-50 flex items-center justify-center shadow-[0_4px_12px_rgba(16,185,129,0.15)]">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center shadow-inner">
                <svg
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 leading-relaxed">
                作品に付けられたタグから、このクリエイターの得意分野や専門領域を可視化しています。
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-3">
            {topTags.slice(0, 15).map(([tag, count]) => (
              <div
                key={tag}
                className="group relative inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200 shadow-[0_4px_12px_rgba(16,185,129,0.08)] hover:shadow-[0_8px_20px_rgba(16,185,129,0.15)] transition-all duration-300"
              >
                <span className="text-sm font-semibold text-emerald-700">
                  #{tag}
                </span>
                <span className="inline-flex items-center justify-center min-w-[24px] h-6 px-2 rounded-full bg-emerald-600 text-white text-xs font-bold shadow-inner">
                  {count}
                </span>
              </div>
            ))}
