    ...
    <div className={containerClass}>
      <div className={cardClass}>
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className={`font-bold text-gray-900 flex items-center gap-2 ${variant === 'toast' ? 'text-lg' : 'text-xl'}`}>
            <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.367 2.684 3 3 0 00-5.367-2.684z" />
            </svg>
            {variant === 'toast' ? 'Xでシェアしますか？' : '共有メッセージを確認'}
          </h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-2">
            <p className="text-sm text-gray-700">
              {variant === 'toast' ? 'ワンクリックで簡単共有！' : '以下の内容でX（Twitter）に共有します。'}
            </p>
            {variant === 'modal' && ogImageUrl && (
              // unoptimized を付与して動的 OGP 画像の読み込み失敗を防ぐ
              <Image src={ogImageUrl} alt="シェア画像" width={600} height={315} className="w-full rounded-lg border border-gray-200" unoptimized />
            )}
