...
export function useWorkCategories(
  ...
  const validateCategoryName = (
    ...
    if (existingCategory) {
      return {
        isValid: false,
        error: `「${existingCategory.name}」と同じ名前のカテゴリが既に存在します`,
      };
    }
      ...
      if (oldName !== newName && category.works.length > 0) {
        ...
        for (const work of category.works) {
          const updatedCategories =
            work.categories?.map((cat) => (cat === oldName ? newName : cat)) ||
            [];
        ...
        const updatedWorks = savedWorks.map((work) => {
          if (work.categories?.includes(oldName)) {
            return {
              ...work,
              categories: work.categories.map((cat) =>
  ...
  const deleteCategory = async (categoryId: string) => {
    try {
      ...
      if (category.works.length > 0) {
        ...
        for (const work of category.works) {
          const updatedCategories =
            work.categories?.filter((cat) => cat !== category.name) || [];
        ...
        const updatedWorks = savedWorks.map((work) => {
          if (work.categories?.includes(category.name)) {
            return {
              ...work,
              categories: work.categories.filter(
  ...
  const updateWorkCategory = async (
    workId: string,
    ...
    try {
      ...
      await fetcher(`/api/works/${workId}/category`, {
        ...
        body: JSON.stringify({
          categories:
            newCategoryName === "未分類" || newCategoryName === ""
              ? []
              : [newCategoryName],
        }),
      });
