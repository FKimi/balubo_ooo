...
export function useWorkStatistics(works: WorkData[]): WorkStatistics {
  return useMemo(() => {
    ...
    const totalWordCount = works
      .filter(
        (work) => work.content_type === "article" && work.article_word_count,
      )
    ...
    const totalRoles = Object.values(roleCount).reduce(
      (sum, count) => sum + count,
      0,
    );
    ...
    const monthlyActivity = Object.values(monthlyActivityMap).sort((a, b) => {
    ...
    const yearlyActivity = Object.values(yearlyActivityMap).sort(
      (a, b) => b.year - a.year,
    );
    ...
    const mostActiveYear =
      yearlyActivity.length > 0
        ? yearlyActivity.reduce((max, current) =>
            current.count > max.count ? current : max,
          )
        : null;
