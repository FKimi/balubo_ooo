...
export function StrengthProfileSection({ works }: StrengthProfileProps) {
  const analysis = useMemo(() => {
    ...
    const types = works
      .map((w) => (w.content_type || "").toLowerCase())
      .filter(Boolean);
    const uniqueTypes = new Set(types).size;
    const tools = works
      .flatMap((w) =>
        (w.design_tools || []).map((t) => String(t).toLowerCase()),
      )
      .filter(Boolean);
    ...
    const avgDescLen =
      descriptions.length > 0
        ? Math.round(descriptions.join("").length / descriptions.length)
        : 0;

    const recent90Days = (() => {
      const threshold = new Date();
      threshold.setDate(threshold.getDate() - 90);
      return works.filter((w) => new Date(w.created_at || "") > threshold)
        .length;
    })();
  ...
  // ペルソナ判定（決定論）
  const persona = (() => {
    if (recent90Days >= 4 || totalWorks >= 12)
      return "継続的にアウトプットするプロダクティブ型";
    if (uniqueTypes >= 4 || uniqueTools >= 6)
      return "多分野に対応するジェネラリスト型";
    if (avgDescLen >= 150)
      return "言語化とドキュメンテーションに強いプランナー型";
  ...
  if (recent90Days >= 3)
    strengths.push("継続的な制作活動で安定したアウトプット力を持っている。");
  if (uniqueTypes >= 3)
    strengths.push("複数分野にわたる柔軟な適応力と横断的な視点を備えている。");
  if (uniqueTools >= 5)
    strengths.push(
      "新しいツールや技術への習得意欲が高く、学習能力に長けている。",
    );
  if (avgViews >= 100)
    strengths.push(
      "制作物が外部から評価されやすく、成果を可視化する力が強い。",
    );
  if (avgDescLen >= 120)
    strengths.push("作品の背景や意図を丁寧に言語化し、伝達力に優れている。");
  if (strengths.length === 0)
    strengths.push(
      "厳選した作品で安定した品質を実現する、確実性の高い制作スタイル。",
    );
  ...
  if (recent90Days < 2)
    growth.push(
      "制作の更新頻度を上げ、小さなアウトプットを積み重ねることで継続力を強化。",
    );
  if (uniqueTypes <= 2)
    growth.push("隣接領域の案件に挑戦し、応用範囲を広げることで多様性を獲得。");
  if (avgDescLen < 80)
    growth.push("作品の背景や成果を丁寧に言語化し、伝達力を向上させる。");
  if (uniqueTools < 4)
    growth.push(
      "最新のワークフローやツールを積極的に取り入れ、制作効率を向上させる。",
    );
  if (avgViews < 80)
    growth.push(
      "作品の訴求力を高め、より多くの人に価値を届けられるようにする。",
    );
  ...
  if (
    types.some((t) =>
      ["web", "website", "landing", "lp"].some((w) => t.includes(w)),
    )
  ) {
  ...
  if (
    types.some((t) =>
      ["ui", "ux", "app", "dashboard"].some((w) => t.includes(w)),
    )
  ) {
  ...
  if (
    types.some((t) =>
      ["branding", "logo", "identity"].some((w) => t.includes(w)),
    )
  ) {
      ...
      <CardContent className="space-y-5">
        ...
        <div className="bg-gradient-to-r from-white to-gray-50 p-4 rounded border border-gray-200">
          <p className="text-sm text-gray-800 leading-relaxed">
            {persona}。
            {workSpanMonths > 0
              ? `約${workSpanMonths}ヶ月にわたる制作活動を通じて、`
              : ""}
            {totalWorks > 0 ? `${totalWorks}作品の制作実績` : "制作活動"}
            を積み重ねている。
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {topTags.slice(0, 3).map((tag, i) => (
            <span
              key={i}
              className="inline-flex items-center rounded-full border border-blue-200 bg-white px-3 py-1 text-xs text-gray-700"
            >
              {tag}
            </span>
          ))}
