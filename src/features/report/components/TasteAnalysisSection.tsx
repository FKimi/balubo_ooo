...
export function TasteAnalysisSection({ works }: TasteAnalysisProps) {
  ...
  const analyzeTaste = (): TasteMetrics => {
    ...
    const avgDescLength =
      descriptions.length > 0 ? totalDescLength / descriptions.length : 0;
    ...
    if (technicalCount > emotionalCount) {
      const dominance = Math.min(
        1,
        (technicalCount - emotionalCount) / Math.max(technicalCount, 1),
      );
      logicalScore = 2 + dominance * 3; // 2-5: ロジカル寄り
    } else if (emotionalCount > technicalCount) {
      const dominance = Math.min(
        1,
        (emotionalCount - technicalCount) / Math.max(emotionalCount, 1),
      );
    ...
    const modernIndex = Math.min(
      1,
      Math.max(0, 0.6 * recentRatio + 0.4 * modernTechRatio),
    ); // 0=クラシック,1=モダン
    // 2(モダン)〜10(クラシック)へ反転マッピング
    modernScore = 2 + (1 - modernIndex) * 8;

    return {
      formal_casual: Math.min(
        10,
        Math.max(1, Math.round(formalScore * 10) / 10),
      ),
      minimal_decorative: Math.min(
        10,
        Math.max(1, Math.round(minimalScore * 10) / 10),
      ),
      logical_emotional: Math.min(
        10,
        Math.max(1, Math.round(logicalScore * 10) / 10),
      ),
      modern_classic: Math.min(
        10,
        Math.max(1, Math.round(modernScore * 10) / 10),
      ),
          ...
          <div key={index} className="space-y-2">
            <div className="flex items-center justify-between text-xs text-gray-600">
              ...
              <span className="text-gray-900 font-medium">
                {showDecimal ? scale.value.toFixed(1) : Math.round(scale.value)}
              </span>
            ...
            <div className="relative">
              <div className="w-full bg-gray-200 rounded-full h-1.5">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, Math.max(0, (scale.value / 10) * 100))}%`,
                  }}
                />
