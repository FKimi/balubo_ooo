...
export function InsightsSection({ works, inputs, workStats }: InsightsSectionProps) {
  ...
  const generateCreatorSummary = () => {
    const topRole = workStats.roleDistribution.length > 0 ? workStats.roleDistribution[0].role : 'クリエイター'
    ...
    const monthsActive = workDates.length > 1 && workDates[0] && workDates[workDates.length - 1]
      ? Math.max(1, Math.round((workDates[workDates.length - 1]!.getTime() - workDates[0]!.getTime()) / (1000 * 60 * 60 * 24 * 30)))
      : 1

    // 主要なコンテンツタイプ
    const contentTypes = works.reduce((acc: {[key: string]: number}, work) => {
      const type = work.content_type || 'その他'
      acc[type] = (acc[type] || 0) + 1
      return acc
    }, {})
    const topContentType = Object.entries(contentTypes).sort(([,a], [,b]) => b - a)[0]?.[0] || 'コンテンツ'

    // インプットの特徴
    let inputCharacteristic = ''
    if (hasInputs) {
      const inputTypes = inputs.reduce((acc: {[key: string]: number}, input) => {
        acc[input.type] = (acc[input.type] || 0) + 1
        return acc
      }, {})
      const topInputType = Object.entries(inputTypes).sort(([,a], [,b]) => b - a)[0]?.[0] || 'book'
      const inputTypeJa = topInputType === 'book' ? '書籍' : topInputType === 'movie' ? '映画' : topInputType === 'other' ? '多様なメディア' : 'コンテンツ'
      inputCharacteristic = `また、${inputsCount}件の${inputTypeJa}インプットを通じて継続的な学習を行い、`
    }

    // 活動の一貫性
    const consistencyNote = monthsActive >= 3 ? `${monthsActive}ヶ月間にわたって継続的に活動し、` : '精力的に活動し、'
        ...
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-gray-800 mb-2">クリエイター傾向</h3>
        ...
        <CardContent className="pt-0">
          <div className="text-center">
            <p className="text-sm text-gray-600">
              過去6ヶ月で{monthlyData.filter(m => m.total > 0).length}ヶ月間活動実績があります
            </p>
          </div>
        </CardContent>
      </Card>
