...
export default function AdvancedMetricsGrid({ metrics }: Props) {
  ...
  const renderCard = (key: string, detail: MetricDetail) => (
    <Card key={key} className="hover:shadow-md transition-shadow duration-200 flex flex-col">
      <div className={`px-6 py-4 bg-gradient-to-r ${colorMap[key] || 'from-slate-500 to-slate-600'} text-white`}>
      ...
      <CardContent className="p-4 space-y-2 flex-grow flex flex-col">
        ...
        {detail.goodHighlight && (
          <p className="text-xs text-slate-600 line-clamp-3">{detail.goodHighlight}</p>
        )}
        <div className="mt-auto">
          {detail.nextTip && (
            <p className="text-[11px] text-blue-600 bg-blue-50 rounded px-2 py-1 inline-block">NEXT: {detail.nextTip}</p>
          )}
        </div>
      </CardContent>
    </Card>
