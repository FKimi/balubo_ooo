...
export function FollowModal({ isOpen, onClose, userId, initialTab }: FollowModalProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [followersRes, followingRes] = await Promise.all([
        fetch(`/api/connections/followers?userId=${userId}`),
        fetch(`/api/connections/following?userId=${userId}`)
      ])

      console.log('フォロワーAPI応答:', followersRes.status, followersRes.ok)
      console.log('フォロー中API応答:', followingRes.status, followingRes.ok)

      if (followersRes.ok) {
        const followersData = await followersRes.json()
        console.log('フォロワーデータ:', followersData)
        setFollowers(followersData)
      } else {
        const errorData = await followersRes.json()
        console.error('フォロワー取得エラー:', errorData)
      }

      if (followingRes.ok) {
        const followingData = await followingRes.json()
        console.log('フォロー中データ:', followingData)
        setFollowing(followingData)
      } else {
        const errorData = await followingRes.json()
        console.error('フォロー中取得エラー:', errorData)
      }
    } catch (error) {
          ...
          {loading ? (
            ...
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-700 mb-2">
                {activeTab === 'followers' ? 'フォロワーがいません' : 'フォローしていません'}
              </h3>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {user.professions && user.professions.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {user.professions.slice(0, 2).map((profession, index) => (
                          <span 
                            key={index}
