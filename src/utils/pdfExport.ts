...
// 旧形式の包括レポート関数（バックアップ）
async function exportComprehensiveReportToPDF_OLD(data: ComprehensiveReportData, fileName?: string) {
  try {
    const pdf = new jsPDF('p', 'mm', 'a4')
    const pageWidth = pdf.internal.pageSize.getWidth()
    const pageHeight = pdf.internal.pageSize.getHeight()
    ...
    // ヘルパー関数：改良された安全な日本語テキスト出力
    const addJapaneseText = (text: string, x: number, y: number, maxWidth?: number) => {
      try {
        // より強力な文字正規化
    ...
    pdf.setFontSize(16)
    pdf.setTextColor(31, 41, 55)
    addJapaneseText('エキスパート認定', margin, currentY)
    currentY += 10

    pdf.setFontSize(14)
    pdf.setTextColor(59, 130, 246)
    addJapaneseText(data.overview.expertLevel, margin, currentY)
    currentY += 10

    pdf.setFontSize(12)
    pdf.setTextColor(34, 197, 94)
    addJapaneseText(`品質スコア: ${data.overview.qualityScore}/100点`, margin, currentY)
    currentY += 15

    // 活動統計
    pdf.setFontSize(16)
    pdf.setTextColor(31, 41, 55)
    addJapaneseText('活動統計', margin, currentY)
    currentY += 10
    ...
    const statBoxHeight = 25
         ...
         // 統計ボックス
     const stats = [
       { label: '総作品数', value: data.overview.totalWorks.toString(), color: [59, 130, 246] as [number, number, number], bg: [239, 246, 255] as [number, number, number] },
       { label: '総文字数', value: data.overview.totalWordCount.toLocaleString(), color: [34, 197, 94] as [number, number, number], bg: [240, 253, 244] as [number, number, number] },
       { label: '平均文字数', value: data.overview.avgWordCount.toLocaleString(), color: [168, 85, 247] as [number, number, number], bg: [245, 243, 255] as [number, number, number] },
       { label: '品質率', value: `${Math.round(data.overview.contentQualityRate)}%`, color: [245, 158, 11] as [number, number, number], bg: [255, 251, 235] as [number, number, number] }
     ]

     stats.forEach((stat, index) => {
       const x = margin + (index % 2) * (statBoxWidth + 10)
       const y = currentY + Math.floor(index / 2) * (statBoxHeight + 10)
       
       pdf.setFillColor(stat.bg[0], stat.bg[1], stat.bg[2])
       pdf.rect(x, y, statBoxWidth, statBoxHeight, 'F')
       pdf.setTextColor(stat.color[0], stat.color[1], stat.color[2])
       pdf.setFontSize(16)
       addJapaneseText(stat.value, x + 5, y + 12)
       pdf.setFontSize(10)
       pdf.setTextColor(75, 85, 99)
       addJapaneseText(stat.label, x + 5, y + 20)
     })

    currentY += 60

    // 強み・特徴
    if (data.overview.strengths.length > 0) {
      checkPageBreak(30)
      pdf.setFontSize(16)
      pdf.setTextColor(31, 41, 55)
      addJapaneseText('強み・特徴', margin, currentY)
      ...
      pdf.setFontSize(11)
      pdf.setTextColor(75, 85, 99)
      data.overview.strengths.forEach(strength => {
        checkPageBreak()
        currentY += addJapaneseText(`• ${strength}`, margin, currentY, pageWidth - 2 * margin)
        currentY += 3
      })
    }
    ...
    pdf.setFontSize(20)
    pdf.setTextColor(44, 82, 130)
    addJapaneseText('作品分析', margin, currentY)
    currentY += 15

    // 作品品質指標
    pdf.setFontSize(16)
    pdf.setTextColor(31, 41, 55)
    addJapaneseText('作品品質指標', margin, currentY)
    currentY += 10

    pdf.setFontSize(11)
    pdf.setTextColor(75, 85, 99)
    currentY += addJapaneseText(`平均文字数: ${data.worksAnalysis.qualityMetrics.avgWordCount.toLocaleString()}文字`, margin, currentY)
    currentY += 5
    currentY += addJapaneseText(`完成率: ${Math.round(data.worksAnalysis.qualityMetrics.completionRate)}%`, margin, currentY)
    currentY += 5
    currentY += addJapaneseText(`タグ多様性: ${data.worksAnalysis.qualityMetrics.tagVariety}種類`, margin, currentY)
    currentY += 15

    // 主要作品一覧
    if (data.worksAnalysis.works.length > 0) {
      pdf.setFontSize(16)
      pdf.setTextColor(31, 41, 55)
      addJapaneseText('主要作品一覧', margin, currentY)
      currentY += 10

      const displayWorks = data.worksAnalysis.works.slice(0, 8)
      ...
      displayWorks.forEach((work, index) => {
        ...
        if (work.description) {
          const shortDesc = work.description.length > 100 ? work.description.substring(0, 100) + '...' : work.description
          currentY += addJapaneseText(shortDesc, margin, currentY, pageWidth - 2 * margin)
          currentY += 3
        }

        if (work.production_date) {
          const date = new Date(work.production_date).toLocaleDateString('ja-JP')
          currentY += addJapaneseText(`制作日: ${date}`, margin, currentY)
          currentY += 3
        }

        if (work.tags && work.tags.length > 0) {
          const tags = work.tags.slice(0, 5).join(', ')
          currentY += addJapaneseText(`タグ: ${tags}`, margin, currentY, pageWidth - 2 * margin)
        }
    ...
    pdf.setFontSize(20)
    pdf.setTextColor(44, 82, 130)
    addJapaneseText('インプット分析', margin, currentY)
    currentY += 15

    // 学習洞察
    pdf.setFontSize(16)
    pdf.setTextColor(31, 41, 55)
    addJapaneseText('学習洞察', margin, currentY)
    currentY += 10

    pdf.setFontSize(11)
    pdf.setTextColor(75, 85, 99)
    currentY += addJapaneseText(`総インプット数: ${data.inputsAnalysis.learningInsights.totalInputs}件`, margin, currentY)
    currentY += 5
    if (data.inputsAnalysis.learningInsights.avgRating > 0) {
      currentY += addJapaneseText(`平均評価: ${data.inputsAnalysis.learningInsights.avgRating.toFixed(1)}点`, margin, currentY)
      currentY += 5
    }
    if (data.inputsAnalysis.learningInsights.topGenres.length > 0) {
      currentY += addJapaneseText(`注目ジャンル: ${data.inputsAnalysis.learningInsights.topGenres.slice(0, 5).join(', ')}`, margin, currentY, pageWidth - 2 * margin)
    }
    currentY += 15

    // 最近のインプット
    if (data.inputsAnalysis.inputs.length > 0) {
      pdf.setFontSize(16)
      pdf.setTextColor(31, 41, 55)
      addJapaneseText('最近のインプット', margin, currentY)
      currentY += 10

      const recentInputs = data.inputsAnalysis.inputs.slice(0, 6)
      ...
      recentInputs.forEach((input, index) => {
        ...
        pdf.setFontSize(12)
        pdf.setTextColor(31, 41, 55)
        currentY += addJapaneseText(input.title, margin, currentY, pageWidth - 2 * margin)
        currentY += 3
        ...
        pdf.setTextColor(107, 114, 128)
                 ...
                 if (input.review) {
           const shortReflection = input.review.length > 80 ? input.review.substring(0, 80) + '...' : input.review
           currentY += addJapaneseText(shortReflection, margin, currentY, pageWidth - 2 * margin)
           currentY += 3
         }
        ...
        if (input.genres && input.genres.length > 0) {
          ...
          currentY += addJapaneseText(`ジャンル: ${genres}`, margin, currentY, pageWidth - 2 * margin)
        }
        
        currentY += 10

        if (index < recentInputs.length - 1) {
          pdf.setDrawColor(229, 231, 235)
          pdf.line(margin, currentY, pageWidth - margin, currentY)
          currentY += 5
        }
    ...
    pdf.setFontSize(20)
    pdf.setTextColor(44, 82, 130)
    addJapaneseText('成長の軌跡', margin, currentY)
    currentY += 15

    // 活動タイムライン
    pdf.setFontSize(16)
    pdf.setTextColor(31, 41, 55)
    addJapaneseText('活動タイムライン', margin, currentY)
    currentY += 10

    if (data.growthInsights.timeline.length > 0) {
      const recentEvents = data.growthInsights.timeline.slice(0, 10)
      
      recentEvents.forEach((event, index) => {
        checkPageBreak(15)
        
                 pdf.setFontSize(10)
         const eventColor = event.type === 'work' ? [34, 197, 94] as [number, number, number] : [168, 85, 247] as [number, number, number]
         pdf.setTextColor(eventColor[0], eventColor[1], eventColor[2])
        currentY += addJapaneseText(`${event.date}`, margin, currentY)
        
        pdf.setTextColor(75, 85, 99)
        currentY += addJapaneseText(`${event.event}`, margin + 30, currentY - lineHeight, pageWidth - margin - 30)
        currentY += 5
      })
    }
    ...
    // 生産性トレンド
    if (data.growthInsights.productivityTrends.length > 0) {
      checkPageBreak(40)
      pdf.setFontSize(16)
      pdf.setTextColor(31, 41, 55)
      addJapaneseText('月別活動量', margin, currentY)
      currentY += 10

      const recentTrends = data.growthInsights.productivityTrends.slice(-6)
      pdf.setFontSize(10)
      pdf.setTextColor(75, 85, 99)
      
      recentTrends.forEach(trend => {
        checkPageBreak()
        currentY += addJapaneseText(`${trend.month}: 作品${trend.works}件、インプット${trend.inputs}件`, margin, currentY)
        currentY += 5
      })
    }
    ...
    // PDFを保存
    const defaultFileName = `${data.profile.displayName}_包括レポート_${new Date().toISOString().split('T')[0]}.pdf`
    pdf.save(fileName || defaultFileName)
    
    return true
  } catch (error) {
    console.error('Comprehensive PDF export error:', error)
    throw new Error('包括レポートPDFの出力に失敗しました')
  }
}
