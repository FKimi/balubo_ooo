...
// 包括的レポート用のHTML生成関数
export function generateComprehensiveReportHTML(
    ...
    <div style="font-family: system-ui, -apple-system, sans-serif; max-width: 800px; margin: 0 auto; background: white;">
      ...
      <div style="padding: 40px; page-break-after: always;">
        ...
        <div style="margin-bottom: 30px;">
          ...
          ${Object.entries(data.inputsAnalysis.genrePreferences)
            .slice(0, 5)
            .map(
              ([genre, count]) => `
      ...
      <div style="padding: 40px;">
        ...
        <div>
          ...
          ${data.growthInsights.skillEvolution
            .slice(0, 5)
            .map(
              (skill) => `
...
export async function exportToPDF(data: ExportData, fileName?: string) {
  try {
    ...
    pdf.setFontSize(20);
    addJapaneseText(
      data.stats.totalWorks.toString(),
      margin + 5,
      currentY + 12,
    );
    pdf.setFontSize(10);
    ...
    pdf.setFontSize(20);
    addJapaneseText(
      data.stats.totalWordCount.toLocaleString(),
      margin + statBoxWidth + 15,
      currentY + 12,
    );
    pdf.setFontSize(10);
    pdf.setTextColor(75, 85, 99);
    ...
    if (data.stats.topTags.length > 0) {
      currentY += addJapaneseText(
        `主要タグ: ${data.stats.topTags.slice(0, 8).join(", ")}`,
        margin,
        currentY,
        pageWidth - 2 * margin,
      );
    ...
    if (data.stats.topGenres.length > 0) {
      currentY += addJapaneseText(
        `好きなジャンル: ${data.stats.topGenres.slice(0, 6).join(", ")}`,
        margin,
        currentY,
        pageWidth - 2 * margin,
      );
...
export async function exportComprehensiveReportToPDF(
  data: ComprehensiveReportData,
  fileName?: string,
) {
  try {
    ...
    const finalFileName =
      fileName ||
      `ユーザー_包括レポート_${new Date().toISOString().split("T")[0]}.pdf`;

    // HTML to PDF変換
    await exportHTMLToPDF(htmlContent, finalFileName, {
      width: 800,
      height: 1200,
    });
...
export async function exportScreenshotToPDF(
  elementId: string,
  fileName?: string,
) {
  try {
